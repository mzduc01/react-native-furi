...
class FuriWord extends React.Component {
    render() {
        ...
        return !this.props.furi
          ...
          <Text style={[{ fontSize: size }, this.props.textStyle, this.props.valueStyle]}>{this.props.value}</Text>
        </View>)
          ...
          <View style={{ borderColor: 'red', borderWidth: 0, flexDirection: 'row', justifyContent: value.length > 1 ? 'space-between' : 'center' }}>
            {[...value].map((item, index) => (<Text key={index} style={[{ fontSize: size }, this.props.textStyle, this.props.valueStyle]}>{item}</Text>))}
          </View>
        </View>);
    }
}
    ...
    render() {
        ...
        const array = (this.props.mode === 'right'
            ? nv.map((item, index) => (item.furi && this.props.showFuri
                ? <Text key={index}><Text style={[this.props.textStyle, this.props.valueStyle]}>{item.value}</Text><Text style={[{ fontSize: furiSize }, this.props.furiStyle]}>{item.furi}</Text></Text>
                : <Text key={index} style={[this.props.textStyle, this.props.valueStyle]}>{item.value}</Text>))
            : nv.map((item, index) => ((item.value === '\n'
                ? <View key={index} style={{ width: '100%', height: 10 }}/>
                : <FuriWord key={index} value={item.value} size={this.props.size} furi={item.furi} style={{ ...this.props.style }} textStyle={this.props.textStyle} valueStyle={{ ...this.props.valueStyle }} furiStyle={{ ...this.props.furiStyle }} showFuri={this.props.showFuri}/>))));
